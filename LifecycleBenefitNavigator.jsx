...
const STAGES = [
  ...
  { id: "middle", name: "중장년", min: 35, max: 64, color: "#CE6B47" },
...
const BENEFITS = [
  ...
  { id: 3, name: "첫만남이용권", ministry: "보건복지부", ageMin: 0, ageMax: 0, incomeCap: 5, vtype: "voucher", valMin: 200, valMax: 300, valNote: "일시금", amount: "200~300만원 바우처", desc: "출생아 1인당 바우처 지급", channel: "복지로·행정복지센터", portal: "복지로", budget: "4,111억원 ('26 열린재정)", src: true, law: "저출산·고령사회기본법", brackets: [["첫째", "200만원"], ["둘째 이상", "300만원"]] },
  { id: 4, name: "부모급여", excl: "infantCare", ministry: "보건복지부", ageMin: 0, ageMax: 1, incomeCap: 5, vtype: "cash", valMin: 600, valMax: 1200, amount: "월 50~100만원", desc: "영아 양육가구 현금 지원", channel: "복지로", portal: "복지로", budget: "2조 3,726억원 ('26 열린재정)", src: true, law: "아동수당법", brackets: [["0세", "월 100만원"], ["1세", "월 50만원"], ["어린이집 이용 시", "보육료 차감 후 차액 지급"]] },
  { id: 5, name: "아동수당", ministry: "보건복지부", ageMin: 0, ageMax: 8, incomeCap: 5, vtype: "cash", valMin: 120, valMax: 156, amount: "월 10만~13만원 (지역별)", desc: "'26년 만 9세 미만으로 확대 — 매년 1세씩 늘어 '30년 12세까지", channel: "복지로", portal: "복지로", budget: "2조 4,822억원 ('26 열린재정, 연령확대로 26.7% 증액)", src: true, law: "아동수당법 ('26.1월 개정)", brackets: [["연령", "만 9세 미만 (매년 1세씩 확대 → '30년 만 12세)"], ["수도권", "월 10만원"], ["비수도권", "월 10.5만원 ('26 한시 가산)"], ["인구감소 우대·특별지역", "월 11만~12만원 ('26 한시 가산)"], ["인구감소지역 상품권 수령 시", "월 최대 13만원 (지자체별)"]] },
  { id: 6, name: "보육료·유아학비 지원", excl: "infantCare", ministry: "교육부(유보통합)", ageMin: 0, ageMax: 5, incomeCap: 5, vtype: "voucher", valMin: 240, valMax: 400, amount: "어린이집·유치원 이용료", desc: "누리과정 등 보육·교육비 지원 (5세 무상교육 단계 확대)", channel: "복지로", portal: "복지로", budget: "6조 5,090억원 ('26 열린재정, 영유아특별회계 보육료+유아교육비)", src: true, law: "유아교육법·영유아보육법", brackets: [["3~5세 누리과정", "월 28만원 수준(예시)"], ["0~2세 보육료", "연령별 차등 단가"]] },
  ...
  { id: 11, name: "국가장학금(Ⅰ·Ⅱ유형)", excl: "scholarship", ministry: "교육부", ageMin: 19, ageMax: 39, incomeCap: 4, reqTags: ["대학(원) 재학"], vtype: "voucher", valMin: 100, valMax: 700, amount: "소득구간별 등록금 지원", desc: "대학 등록금 부담 경감", channel: "한국장학재단", portal: "한국장학재단", budget: "맞춤형 국가장학금 지원 5조 1,161억원 ('26 열린재정)", src: true, law: "한국장학재단 설립 등에 관한 법률", brackets: [["기초·차상위", "등록금 전액(한도 내)"], ["1~3구간", "연 570만원 수준(예시)"], ["4~6구간", "연 420만원 수준(예시)"], ["7~8구간", "연 350만원 수준(예시)"], ["9구간", "연 100만원 수준(예시)"]] },
  ...
  { id: 13, name: "국가근로장학금", ministry: "교육부", ageMin: 19, ageMax: 39, incomeCap: 3, reqTags: ["대학(원) 재학"], vtype: "cash", valMin: 100, valMax: 500, amount: "교내외 근로 장학", desc: "근로 경험과 장학금 동시 제공", channel: "한국장학재단", portal: "한국장학재단", budget: "맞춤형 국가장학금 지원 5조 1,161억원 내 ('26)", src: true, law: "한국장학재단 설립 등에 관한 법률", brackets: [["교내 근로", "시급 1만원 내외(예시)"], ["교외 근로", "시급 1.2만원 내외(예시)"]] },
  ...
  { id: 21, name: "자녀장려금", ministry: "국세청", ageMin: 19, ageMax: 64, incomeCap: 3, reqTags: ["자녀 양육"], vtype: "cash", valMin: 50, valMax: 100, valNote: "자녀 1인 기준", amount: "자녀 1인당 최대 100만원", desc: "저소득 양육가구 지원", channel: "홈택스", portal: "홈택스", budget: "조세지출 약 1조원(예시)", law: "조세특례제한법", brackets: [["18세 미만 자녀", "1인당 50~100만원(소득별)"]] },
  ...
  { id: 28, name: "장애수당", ministry: "보건복지부", ageMin: 18, ageMax: 100, incomeCap: 1, disability: "mild", vtype: "cash", valMin: 72, valMax: 72, amount: "월 6만원", desc: "경증장애인(심하지 않은 장애) 기초·차상위 대상 수당", channel: "행정복지센터", portal: "복지로", budget: "2,300억원 ('26 열린재정, 기초+차상위)", src: true, law: "장애인복지법", brackets: [["기초·차상위 경증장애인", "월 6만원 수준(예시)"], ["보장시설 수급자", "월 3만원 수준(예시)"]] },
  { id: 29, name: "장애인 활동지원 서비스", ministry: "보건복지부", ageMin: 6, ageMax: 64, incomeCap: 5, disability: "any", vtype: "service", valMin: 0, valMax: 0, amount: "활동지원사 서비스", desc: "일상·사회활동 지원 (서비스 지원 종합조사로 급여량 결정)", channel: "복지로·연금공단", portal: "복지로", budget: "2조 8,164억원 ('26 열린재정)", src: true, law: "장애인활동 지원에 관한 법률", brackets: [["급여량", "종합조사 구간별 월 60~747시간(예시)"], ["본인부담", "소득 수준별 차등(기초 면제)"]] },
  { id: 30, name: "기초연금", excl: "basicOffset", ministry: "보건복지부", ageMin: 65, ageMax: 100, incomeCap: 3, vtype: "cash", valMin: 420, valMax: 480, amount: "월 최대 34.97만원 (저소득 40만원)", desc: "소득하위 70% 어르신 연금 — '26년 저소득층 40만원 우선 인상", channel: "복지로·국민연금공단", portal: "복지로", budget: "23조 1,378억원 ('26 열린재정)", src: true, law: "기초연금법", brackets: [["기준연금액", "월 349,700원 ('26 확정, 물가 2.1% 반영)"], ["저소득(중위 50% 이하)", "월 40만원 ('26 우선 인상 → '27 전체 확대 예정)"], ["선정기준액", "단독 월 247만원·부부 395.2만원 이하 ('26)"], ["부부가구", "각 20% 감액"], ["국민연금 연계", "수급액 따라 감액 가능"]] },
  ...
  { id: 41, name: "자립준비청년 자립수당", ministry: "보건복지부", ageMin: 18, ageMax: 23, incomeCap: 5, reqTags: ["자립준비청년(보호종료)"], vtype: "cash", valMin: 600, valMax: 600, amount: "월 50만원", desc: "아동복지시설·위탁가정 보호종료 후 5년간 자립 지원", channel: "행정복지센터·자립지원전담기관", portal: "복지로", budget: "341억원 ('26 열린재정)", src: true, law: "아동복지법", brackets: [["보호종료 후 5년 이내", "월 50만원"], ["자립정착금", "지자체별 1,000만원 내외 별도(예시)"]] },
  { id: 42, name: "아동발달지원계좌(디딤씨앗통장)", ministry: "보건복지부", ageMin: 0, ageMax: 17, incomeCap: 1, vtype: "cash", valMin: 60, valMax: 120, amount: "적립금 1:2 정부 매칭", desc: "취약계층 아동 자산형성 지원 — 만 18세 이후 자립 용도 사용", channel: "행정복지센터", portal: "복지로", budget: "1,553억원 ('26 열린재정, 보조)", src: true, law: "아동복지법", brackets: [["정부 매칭", "본인 적립의 2배, 월 최대 10만원(예시)"], ["대상", "수급가구 아동·보호대상 아동 등"]] },
  { id: 47, name: "무공해차(전기·수소차) 구매보조금", ministry: "기후에너지환경부", ageMin: 19, ageMax: 100, incomeCap: 5, vtype: "cash", valMin: 200, valMax: 580, valNote: "일시금·차종별", amount: "국비 최대 580만원 수준", desc: "전기·수소차 구매 시 보조금 지급 (지방비 별도 추가)", channel: "무공해차 통합누리집·지자체", portal: "정부24", budget: "2조 2,845억원 ('26 열린재정)", src: true, law: "대기환경보전법", brackets: [["전기승용", "국비 최대 580만원 수준(예시) + 지방비"], ["전기화물", "국비 1,000만원 내외(예시)"], ["수소승용", "국비 2,000만원 내외(예시)"]] },
  ...
  { id: 56, name: "다함께돌봄센터(초등돌봄)", ministry: "보건복지부", ageMin: 6, ageMax: 12, incomeCap: 5, vtype: "service", valMin: 0, valMax: 0, amount: "방과후 초등 돌봄", desc: "소득 무관 초등학생 상시·일시 돌봄 (지역 돌봄센터)", channel: "지자체 돌봄센터", portal: "정부24", budget: "775억원 ('26 열린재정, 자율)", src: true, law: "아동복지법", brackets: [["이용료", "월 10만원 내외 자부담(지자체별 상이·예시)"]] },
  ...
  { id: 33, name: "장제급여", ministry: "보건복지부", ageMin: 0, ageMax: 100, incomeCap: 0, vtype: "cash", valMin: 80, valMax: 80, valNote: "일시금", amount: "80만원", desc: "수급자 사망 시 장제비 지급 — 삶의 마지막까지", channel: "행정복지센터", portal: "복지로", budget: "해산·장제급여 519억원 ('26 열린재정)", src: true, law: "국민기초생활보장법", brackets: [["1구당", "80만원"]] },
];
...
export default function LifecycleBenefitNavigator() {
  ...
  const matchesProfile = (b) => {
    ...
    if (incomeRank > b.incomeCap) return false;
    if (b.gender && b.gender !== gender) return false;
    if (b.disability) {
      if (disability === "none") return false;
      if (b.disability !== "any" && b.disability !== disability) return false;
    ...
    if (b.reqTags && !b.reqTags.every((t) => tags.includes(t))) return false;
    if (b.anyTags && !b.anyTags.some((t) => tags.includes(t))) return false;
    return true;
  };

  const profileMatched = useMemo(() => BENEFITS.filter(matchesProfile), [income, tags, gender, disability, vet, mil, region, audience, bizSize, bizTags]);
  ...
  const sumable = nowEligible.filter((b) => b.vtype === "cash" || b.vtype === "voucher");
  ...
  const totalMin = sumPicks.reduce((s, b) => s + b.valMin, 0);
  const totalMax = sumPicks.reduce((s, b) => s + b.valMax, 0);
    ...
    <div style={{ zoom: bigText ? 1.18 : 1, minHeight: "100vh", background: "#F4F6F5", color: "#22303C", fontFamily: "'Pretendard','Apple SD Gothic Neo','Noto Sans KR',sans-serif" }}>
      ...
      <header style={{ padding: "28px 20px 18px", maxWidth: 860, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          <span style={{ fontSize: 11, letterSpacing: 2, background: "#22303C", color: "#fff", padding: "3px 8px", borderRadius: 4 }}>DEMO v3.2</span>
        ...
        <div style={{ fontSize: 11.5, color: "#8B968F", margin: "0 0 14px" }}>
          21개 부처·청 · '26년 세부사업 6,400여 개 분석 · 국민 직접수혜 124개 사업 · 지자체 추가지원 별도
        </div>
      ...
      <main style={{ maxWidth: 860, margin: "0 auto", padding: "0 20px 60px" }}>
        ...
        <section className="card" style={{ marginBottom: 16 }}>
          ...
          {income === "unknown" && (
            <div style={{ fontSize: 12, color: "#8A6D3B", background: "#FBF6E9", borderRadius: 8, padding: "8px 12px", marginBottom: 16 }}>
              우선 중위소득 100% 기준으로 보여드립니다. 실제 심사는 재산을 포함한 '소득인정액' 기준이라 결과가 달라질 수 있으니, 복지로 모의계산으로 확인해 보세요.
            </div>
          )}
        ...
        <section className="card" style={{ marginBottom: 16, background: "#22303C", color: "#fff", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ fontSize: 12.5, color: "#AEBBB4", marginTop: 4 }}>
            지금 조건에서 받을 수 있는 연간 수혜 범위 (현금·바우처 {sumPicks.length}건 합산{dedupCount > 0 && <> · 중복불가 {dedupCount}건 제외</>})
          </div>
        ...
        {audience === "biz" && (
          <section className="card" style={{ marginBottom: 16, background: "#22303C", color: "#fff", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 4, background: spectrum }} />
            <div style={{ fontSize: 12.5, color: "#AEBBB4", marginTop: 4 }}>
              지금 조건에서 받을 수 있는 연간 지원 예상 범위 (장려금·바우처 {bizSumable.length}건 합산)
            </div>
            <div className="serif" style={{ fontSize: 32, fontWeight: 800, margin: "4px 0 2px", lineHeight: 1.2 }}>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 16 }}>
          {[
            ["지금 받을 수 있는 혜택", `${nowEligible.length}건`],
            ["관련 부처·기관", `${ministries.size}곳`],
            ["앞으로 열릴 혜택", `${upcoming.length}건`],
          ].map(([k, v]) => (
        ...
        <section style={{ marginBottom: 20 }}>
          <h2 className="serif" style={{ fontSize: 18, fontWeight: 800, margin: "0 0 10px" }}>
            {audience === "personal" ? "지금 받을 수 있는 혜택 " : "기업·사업주 지원 사업 "}<span style={{ color: curStage.color }}>{nowEligible.length}건</span>
          </h2>
          <div style={{ display: "grid", gap: 10 }}>
            {nowEligible.map((b) => {
              ...
              return (
                <div key={b.id} className="card clickable" onClick={() => setSelected(b)} style={{ borderLeft: `4px solid ${stageOf(b.ageMin).color}` }}>
                  ...
                  <div style={{ fontSize: 11.5, color: "#8B968F", marginTop: 6 }}>
                    신청: {b.channel} · <span style={{ color: stageOf(b.ageMin).color, fontWeight: 700 }}>자세히 보기 →</span>
                  </div>
                </div>
              );
            })}
          ...
          <div onClick={(e) => e.stopPropagation()}
            ...
            <div style={{ marginTop: 14, background: "#F2F7F4", borderRadius: 10, padding: "12px 14px" }}>
              ...
              <div style={{ fontSize: 12.5, lineHeight: 1.7, color: "#3C4A44" }}>
                ...
                ② {selected.channel}에서 신청 (신분증·본인 명의 통장 기본 지참)<br />
